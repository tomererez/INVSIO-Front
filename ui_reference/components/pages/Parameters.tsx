...
const PriceActionVisual = () => {
    const [mode, setMode] = useState<'bullish' | 'bearish'>('bullish');
...
const CVDVisual = () => {
    const [mode, setMode] = useState<'bearish' | 'bullish'>('bearish');

    useEffect(() => {
        const timer = setInterval(() => {
            setMode(prev => prev === 'bearish' ? 'bullish' : 'bearish');
        }, 8000); // 8 second cycle to allow animation to complete and be read
        return () => clearInterval(timer);
    }, []);
    ...
    return (
        <div className="relative w-full h-56 bg-black/20 rounded-xl overflow-hidden flex flex-col border border-white/5 group">
                    ...
                    <AnimatePresence mode="wait">
                        {isBearish ? (
                            <motion.div
                                ...
                                transition={{ delay: 2.8 }} // Delayed until after graph & trendlines
            ...
            <motion.div
                ...
                initial={{ left: "0%" }}
                animate={{ left: "100%" }}
                transition={{ duration: 2, ease: "linear", repeat: Infinity, repeatDelay: 4 }} // Match roughly with the cycle
            />
        </div>
    );
};
...
const RSIVisual = () => {
    ...
    return (
        <div className="relative w-full h-56 bg-black/20 rounded-xl overflow-hidden flex flex-col border border-white/5 group">
                    ...
                    <AnimatePresence mode="wait">
                        {isBearish ? (
                            <motion.g key="bearish-rsi">
                                {/* Bearish Scenario: RSI making Lower Highs */}
                                <motion.path
                            ...
                            <motion.g key="bullish-rsi">
                                {/* Bullish Scenario: RSI making Higher Lows */}
                                <motion.path
            ...
            {/* Scanner Line */}
            <motion.div
                className="absolute top-0 bottom-0 w-px bg-purple-500/50 shadow-[0_0_10px_#a855f7] z-30"
                initial={{ left: "0%" }}
                animate={{ left: "100%" }}
                transition={{ duration: 2, ease: "linear", repeat: Infinity, repeatDelay: 6 }}
            />
        </div>
    );
};
...
const OIVisual = () => {
    const [mode, setMode] = useState<'short_covering' | 'new_money'>('short_covering');
    ...
    return (
        <div className="relative w-full h-56 bg-black/20 rounded-xl overflow-hidden flex flex-col border border-white/5 group">
            ...
            <div className="flex-1 relative flex items-center px-4 border-b border-white/5 bg-gradient-to-b from-white/[0.02] to-transparent">
                ...
                {/* Price Graph - Always Going UP for these scenarios */}
                <svg className="w-full h-full overflow-visible" viewBox="0 0 600 100" preserveAspectRatio="none">
                    <motion.g key="price-up">
                        <motion.path
            ...
            {/* Scanner Line */}
            <motion.div
                className={`absolute top-0 bottom-0 w-px ${isShortCovering ? 'bg-rose-500/50 shadow-[0_0_10px_#f43f5e]' : 'bg-indigo-500/50 shadow-[0_0_10px_#6366f1]'} z-30`}
                initial={{ left: "0%" }}
                animate={{ left: "100%" }}
                transition={{ duration: 2, ease: "linear", repeat: Infinity, repeatDelay: 6 }}
            />
...
const VolumeVisual = () => {
    const [mode, setMode] = useState<'conviction' | 'divergence'>('conviction');
    ...
    return (
        <div className="relative w-full h-64 bg-black/20 rounded-xl overflow-hidden flex flex-col border border-white/5 group">
            ...
            <div className="h-[40%] relative px-4 flex items-end justify-between gap-1 pb-0 bg-black/40">
                ...
                {/* Render Bars */}
                {barData.map((data, i) => {
                    // Determine color and height based on mode
...
const GuideSection = ({ title, sub, icon: Icon, color, children, visual }: any) => (
    <motion.div
...
export const MarketParametersGuide: React.FC = () => {
    return (
        <div className="min-h-screen pt-32 pb-20 px-6 max-w-6xl mx-auto relative">

            {/* Background */}
            <div className="fixed inset-0 pointer-events-none">
                <div className="absolute top-20 right-0 w-[500px] h-[500px] bg-emerald-900/10 rounded-full blur-[120px]" />
            ...
            <div className="mt-32 text-center">
                <GlassCard className="p-10 border-indigo-500/20 bg-indigo-900/5 inline-block">
                    ...
                    <div className="flex justify-center">
                        <button className="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg flex items-center gap-2 transition-colors">
